  ...
  return <>
    ...
    {scan&&<section id="scan-results" className="section"><div className="container">
      <div className="grid grid-2" style={{alignItems:'stretch'}}>
        ...
        <div className="card"><div className="eyebrow">CATEGORY BREAKDOWN</div>{Object.entries(scan.categoryScores).map(([name,value])=><div key={name} style={{marginTop:20}}><div style={{display:'flex',justifyContent:'space-between',gap:16}}><strong>{name}</strong><strong>{value}/100</strong></div><div style={{height:9,background:'rgba(255,255,255,.08)',borderRadius:999,overflow:'hidden',marginTop:8}}><div style={{width:`${value}%`,height:'100%',background:'linear-gradient(90deg,var(--blue),#65f0ff)'}}/></div></div>)}</div>
      </div>
